...
def pair_data(primary: HeteroData, secondary: HeteroData) -> HeteroData:
    def split_graph(nodes: NodeStorage, edges: EdgeStorage) -> (NodeStorage, EdgeStorage, NodeStorage, EdgeStorage):
        keys = list(nodes.to_dict().keys())
        mask = nodes.batch == 0
        size_s = nodes.ptr[1]
        nodes_s = NodeStorage({k: nodes[k][mask] for k in keys if nodes[k].size(0) == len(mask)})
        nodes_t = NodeStorage({k: nodes[k][~mask] for k in keys if nodes[k].size(0) == len(mask)})
    ...
    s, bond_s, t, bond_t = split_graph(paired['ligand'], paired[('ligand', 'bond', 'ligand')])
    del paired['ligand']
    ...
    s, bond_s, t, bond_t = split_graph(paired['pocket'], paired[('pocket', 'bond', 'pocket')])
    del paired['pocket']
...
class KinodataDockedPairs(KinodataDocked):
    def __init__(self, pair_filter: Optional[
            Callable[[tuple[HeteroData, HeteroData]], bool]
        ] = None, **kwargs):
    ...
    def process(self):
        ...
        pair_list = [pair_data(a, b) for a, b in tqdm(filter(self.pair_filter, product(data_list, repeat=2)))]
