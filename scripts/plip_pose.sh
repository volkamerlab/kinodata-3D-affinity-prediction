...
docker run --rm -v ${PWD}:/results -w /results -u $(id -u ${USER}):$(id -g ${USER}) pharmai/plip:latest -tq -o $out_path_id -f data/pdbs/${ident}.pdb
chown -R $user data/interaction_analysis/$ident
